...
from .utils import (_tokenize, get_identifier_posistions_from_code,
                    ...
                    get_masked_code_by_position, CodeDataset)
...
class Greedy(Attack):
    ...
    def __init__(self, model, tokenizer, lang):
        super().__init__("Greedy", model, tokenizer)
        self.lang = lang
        self.model_mlm = RobertaForMaskedLM.from_pretrained("microsoft/codebert-base-mlm")
        self.tokenizer_mlm = RobertaTokenizer.from_pretrained("microsoft/codebert-base-mlm")

    def forward(self, code, label):
    ...
    def generate_substituions(self, code):
        ...
        word_predictions = codebert_mlm(input_ids_.to(self.device))[0].squeeze()  # seq-len(sub) vocab
        ...
        with torch.no_grad():
            orig_embeddings = codebert_mlm.roberta(input_ids_.to(self.device))[0]
        ...
        for tgt_word in names_positions_dict.keys():
            ...
            for one_pos in tgt_positions:
                ...
                for i in range(nums_candis):
                    ...
                    with torch.no_grad():
                        new_embeddings = codebert_mlm.roberta(new_ids_.to(self.device))[0]
